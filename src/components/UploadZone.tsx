...
interface HeaderSearchResult { nameColIndex: number; phoneColIndex: number; dataStartRow: number; headerRowFound: number; }
...
function findHeaders(rawData: unknown[][]): HeaderSearchResult | null {
  for (let r = 0; r < Math.min(HEADER_SCAN_ROWS, rawData.length); r++) {
    ...
    let tempNameIdx = -1; let tempPhoneIdx = -1;
    for (let c = 0; c < Math.min(HEADER_SCAN_COLS, row.length); c++) {
      const cellVal = row[c]; if (cellVal === null || cellVal === undefined || cellVal === '') continue;
      const cellStr = String(cellVal).trim(); if (!cellStr) continue;
      if (tempNameIdx === -1 && fuzzyMatch(cellStr, ['name', 'fullname', 'membername', 'congregant', 'firstname', 'lastname'])) tempNameIdx = c;
      if (tempPhoneIdx === -1 && fuzzyMatch(cellStr, ['phone', 'contact', 'telephone', 'mobile', 'cell', 'phonenumber'])) tempPhoneIdx = c;
    }
    if (tempNameIdx !== -1 && tempPhoneIdx !== -1) return { nameColIndex: tempNameIdx, phoneColIndex: tempPhoneIdx, dataStartRow: r + 1, headerRowFound: r };
  }
  let nameResult: { row: number; col: number } | null = null; let phoneResult: { row: number; col: number } | null = null;
  for (let r = 0; r < Math.min(HEADER_SCAN_ROWS, rawData.length); r++) {
    const row = rawData[r]; if (!row) continue;
    for (let c = 0; c < Math.min(HEADER_SCAN_COLS, row.length); c++) {
      const cellVal = row[c]; if (cellVal === null || cellVal === undefined || cellVal === '') continue;
      const cellStr = String(cellVal).trim(); if (!cellStr) continue;
      if (!nameResult && fuzzyMatch(cellStr, ['name', 'fullname', 'membername', 'congregant', 'firstname', 'lastname'])) nameResult = { row: r, col: c };
      if (!phoneResult && fuzzyMatch(cellStr, ['phone', 'contact', 'telephone', 'mobile', 'cell', 'phonenumber'])) phoneResult = { row: r, col: c };
    }
  }
  if (nameResult && phoneResult) return { nameColIndex: nameResult.col, phoneColIndex: phoneResult.col, dataStartRow: Math.max(nameResult.row, phoneResult.row) + 1, headerRowFound: Math.max(nameResult.row, phoneResult.row) };
  return null;
}
...
export function UploadZone({ onRecordsParsed, onAlert }: UploadZoneProps) {
  ...
  const parseFile = async (file: File) => {
    ...
    if (!filename.endsWith('.csv') && !filename.endsWith('.xlsx') && !filename.endsWith('.xls')) { onAlert('Please upload a CSV or Excel file.', 'error'); return; }
    if (file.size > MAX_FILE_BYTES) { onAlert('File is too large. Please use a file smaller than 5 MB.', 'error'); return; }
    try {
      const buffer = await file.arrayBuffer(); const workbook = XLSX.read(buffer, { type: 'array' }); const worksheet = workbook.Sheets[workbook.SheetNames[0]];
      ...
      for (let r = dataStartRow; r < rawData.length; r++) {
        const row = rawData[r]; if (!row) continue;
        const name = String(row[nameColIndex] ?? '').trim(); const phone = normalizePhone(row[phoneColIndex]);
      ...
      if (transformed.length === 0) { onAlert('Found headers but no usable data.', 'error'); return; }
      await onRecordsParsed(transformed);
      onAlert(`Loaded ${transformed.length.toLocaleString()} records successfully.`, 'success');
    } catch (error) { onAlert(`Error parsing file: ${(error as Error).message}`, 'error'); }
  };

  const handleDrop = async (event: DragEvent<HTMLDivElement>) => { event.preventDefault(); event.currentTarget.classList.remove('dragover'); const file = event.dataTransfer.files[0]; if (file) await parseFile(file); };
  ...
  return (
    <div style={{ maxWidth: '650px', margin: '0 auto', padding: '1rem' }}>
      <div
        ...
        style={{ borderRadius: '12px', padding: '2.5rem 1.5rem', textAlign: 'center', cursor: 'pointer', transition: 'all 0.2s ease', marginBottom: '2rem' }}
      >
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.8rem', background: 'var(--bg-input)', padding: '0.4rem 0.8rem', borderRadius: '20px', border: '1px solid var(--border)', color: 'var(--neutral-400)' }}>
          <FileSpreadsheet size={14} style={{ color: 'var(--success)' }} />
          <span>Smart scan finds headers even with title rows or merged cells</span>
        </div>
      </div>

      <div className="upload-guide-box" style={{ borderRadius: '12px', padding: '1.5rem' }}>
        <h3 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: '1rem', color: 'var(--neutral-500)', borderBottom: '1px solid var(--border)', paddingBottom: '0.5rem' }}>💡 How to Use This Application</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', background: 'var(--bg-main)', padding: '0.75rem', borderRadius: '8px', border: '1px dashed var(--border)' }}>
