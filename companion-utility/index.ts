...
// Define the shape of your expected AI output records
...
export interface Env {
  AI: {
    run: (model: string, inputs: Record<string, any>) => Promise<{ response?: string }>;
  };
}
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (payload.action === 'ocr') {
        ...
        // Convert base64 representation to binary buffer securely using modern Uint8Array API
      ...
      if (payload.action === 'parse_text') {
        ...
        const modelResponse = await env.AI.run('@cf/meta/llama-3.1-8b-instruct', {
          prompt: `Parse contacts from text list. Return raw JSON array of objects with structure: [{"name":"Name","phone":"CleanNumber"}]. No markdown. Text: ${textPayload.text}`
        });
      ...
      if (payload.action === 'sanitize') {
        ...
        const modelResponse = await env.AI.run('@cf/meta/llama-3.1-8b-instruct', {
          prompt: `Normalize names/phones. Return raw JSON array of objects with structure: [{"name":"Name","phone":"CleanNumber"}]. No markdown. Data: ${JSON.stringify(rowsToProcess)}`
        });
 ...
 */
function sanitizeAndExtractJson(rawText: string): ContactRecord[] {
  ...
  try {
    return JSON.parse(jsonStr) as ContactRecord[];
  } catch (err: any) {
    ...
    try {
      return JSON.parse(cleanedStr) as ContactRecord[];
    } catch (e: any) {
